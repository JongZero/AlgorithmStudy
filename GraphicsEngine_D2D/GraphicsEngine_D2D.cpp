...
{
	RECT rc;
	GetClientRect(m_Hwnd, &rc);	// 클라이언트 영역의 좌표를 얻는다.

	// Render Target을 생성한다.
	HRESULT hr = m_pFactory->CreateHwndRenderTarget(RenderTargetProperties(),
		HwndRenderTargetProperties(m_Hwnd, SizeU(rc.right - rc.left, rc.bottom - rc.top)),
		&m_pRenderTarget);
